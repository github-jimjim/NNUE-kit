...
fn eval_nnue(fen: &str) -> PyResult<f32> {
    let model = MODEL.get().ok_or_else(|| 
        pyo3::exceptions::PyValueError::new_err("Modell nicht initialisiert! Bitte zuerst init_nnue(nnue_path) aufrufen.")
    )?;
...
fn dense_layer(input: &[i32], weights: &[i8], biases: &[i32], in_dim: usize, out_dim: usize) -> Vec<i32> {
...
fn nnue_relu(x: i32) -> i32 {
    ...
    if y < 0 { 0 } else if y > 127 { 127 } else { y }
}
